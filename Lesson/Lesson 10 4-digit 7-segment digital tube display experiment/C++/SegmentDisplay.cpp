...
void SegmentDisplay::DisplayChar(unsigned char n)
{
    unsigned char ch = dight_display[n];
    if (n < 10) {
...
void SegmentDisplay::DisplayChar(int n)
{
    ...
    if (mode == SEGMENT_DISPLY_4BIT) {
        numble2dis(n);
        for(int i = 0; i < 4; i++) {
            DisplayChar((unsigned char)display_array[i]);
...
void SegmentDisplay::DisplayChar(unsigned char sel, unsigned char n)
{
    unsigned char ch = dight_display[n];
    for (unsigned char i = 0; i < 8; i++) {
        if (ch & (1 << i)) {
